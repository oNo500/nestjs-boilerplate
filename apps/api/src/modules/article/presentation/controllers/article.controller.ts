...
export class ArticleController {
  ...
  async addTag(@Param('id') id: string, @Body() dto: AddTagDto): Promise<ArticleResponseDto> {
    try {
  ...
  @ApiResponse({ status: 404, description: 'Article not found' })
  async removeTag(@Param('id') id: string, @Param('tag') tag: string): Promise<ArticleResponseDto> {
  ...
  @UseEnvelope()
  @ApiOperation({
    summary: 'Get article list (cursor pagination; suitable for large or real-time datasets)',
  })
